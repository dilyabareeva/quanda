...
from quanda.benchmarks.heuristics import MixedDatasets, TopKCardinality, ModelRandomization
from quanda.benchmarks.ground_truth import LinearDatamodeling
from quanda.benchmarks.downstream_eval import MislabelingDetection, ClassDetection, ShortcutDetection, SubclassDetection
...
def main(benchmarks_path, cache_dir):
    ...
    for f in files:
        if not f.endswith(".pth"):
            break
        bench_state = torch.load(os.path.join(benchmarks_path, f),map_location=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ignore", nargs="+", default=[], type=int, help="Ignore specific benchmarks")
    parser.add_argument("--cache_dir", required=False, type=str, default=None, help="Directory to use as benchmark download cache")
