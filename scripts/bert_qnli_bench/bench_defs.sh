...
BENCH_PARAMS[MislabelingDetection]="train_dataset=qnli_train_mislabeling train_dataset.dataset_split='train' eval_dataset=qnli_test eval_dataset.dataset_split='validation' model.trainer.optimizer=adamw model.trainer.lr=2e-5 model.trainer.max_epochs=20 model.trainer.scheduler_kwargs.end_factor=0.1 device=cuda:1 hydra.launcher.n_jobs=3"
