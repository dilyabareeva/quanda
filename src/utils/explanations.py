...
class Explanations:
    def __init__(
        ...
        cache_batch_size: Optional[int] = 32,
        cache_path: Optional[str] = None,
    ):
        assert not cache or (
            cache_batch_size is not None and cache_path is not None
        ), "Either set cache=False or set positive integer cache_batch_size and a cache_path."
    ...
    def add(self, explanation_targets: torch.Tensor, explanations: torch.Tensor):
        assert len(explanations.shape) == 3, f"Explanations object has {len(explanations.shape)} != 2 dimensions"
        assert explanations.shape[-1] == len(
            self.train_dataset
        ), f"Given explanations are {explanations.shape[-1]} != {len(self.train_dataset)} dimensional."
        assert (
