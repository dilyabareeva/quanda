...
def test_representer_points_explain(
    ...
    explainer = RepresenterPoints(
        model=model, checkpoints=checkpoint, cache_dir=str(tmp_path), train_dataset=dataset, **method_kwargs
    )
...
def test_representer_points_self_influence(
    test_id, model, checkpoint, dataset, train_labels, method_kwargs, request, tmp_path
):
    model = request.getfixturevalue(model)
    checkpoint = request.getfixturevalue(checkpoint)
    dataset = request.getfixturevalue(dataset)
